...
import { Button, HeaderApartContainer, HeaderWrapper, QuantityCounter, SortButton } from './styles'

const Header = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useAppDispatch();
  const { cart, byName, byPrice, byPriceOrName } = useAppSelector((store) => store.product);
            ...
            {isShowSortButtons &&
              <>
                <SortButton
                  ...
                  type="button"
                >
                  {byPrice > 0 ? <IconSortAscendingNumbers /> : <IconSortDescendingNumbers />}
                </SortButton>
                ...
                <SortButton
                  ...
                  type="button"
                >
                  {byName > 0 ? <IconSortAscendingLetters /> : <IconSortDescendingLetters />}
                </SortButton>
              </>
            }
